#!/bin/bash
#
# CrowdSurfer Shaka Edge Device - Raspberry Pi Deployment Script v4
#
# Version: 4.0.0
...
EDGE_FILES_COPIED=0
for file in telemetry_agent.py management_agent.py portal_handler.py queue.py config.py monitor_heartbeat.py local_admin_server.py requirements.txt; do
...
if [ $EDGE_FILES_COPIED -gt 0 ]; then
    print_success "Copied $EDGE_FILES_COPIED edge file(s) from deployment package"
else
    print_warning "No edge files found in deployment package"
    print_info "After this script completes, copy edge files to: $INSTALL_DIR/edge"
fi
